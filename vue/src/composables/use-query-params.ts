...
export function useQueryParams<ValueType>(
  { options, urlKey } = {} as Options,
): ReturnValue<ValueType> {
  const route = useRoute()

  const value = shallowRef((route.query[urlKey] || options[0]) as ValueType)
  ...
  const toggle = (mode: ValueType) => {
    ...
    const newQueryString = new URLSearchParams(route.query as Record<string, string>).toString()
